...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements DataApi.DataListener,
	...
	public static final int SECONDS_PER_MINUTE = 5;
	public static final int SYNC_INTERVAL_IN_MINUTES = 6;
	...
	public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
		...
		try {
			...
			Uri builtUri = Uri.parse(FORECAST_BASE_URL).buildUpon()
					...
					.appendQueryParameter(APPID_PARAM, "55043ca149550f44459ac5108bf6f7ac") //BuildConfig.OPEN_WEATHER_MAP_API_KEY)
					.build();
	...
	private void getWeatherDataFromJson(String forecastJsonStr,
		...
		try {
			...
			if (cVVector.size() > 0) {
				...
				Cursor cursor = getContext().getContentResolver().query(weatherForLocationUri, null, null, null, null);
				...
				if (cursor != null && cursor.getCount() > 0) {
					...
					weathid = cursor.getInt(cursor.getColumnIndexOrThrow(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID));
	...
	class DataTask extends AsyncTask<Node, Void, Void> {
		...
		protected Void doInBackground(Node... nodes) {
			...
			putDataMapReq.getDataMap().putString(getContext().getString(R.string.tempmax), Utility.formatTemperature(getContext(), maxtemp));
			putDataMapReq.getDataMap().putString(getContext().getString(R.string.tempmin), Utility.formatTemperature(getContext(), mintemp));
			putDataMapReq.getDataMap().putInt(getContext().getString(R.string.weathid), weathid);
			PutDataRequest putDataReq = putDataMapReq.asPutDataRequest();
			putDataReq.setUrgent();
			DataApi.DataItemResult dataItemResult = Wearable.DataApi
					.putDataItem(mGoogleApiClient, putDataReq).await();
			if (dataItemResult.getStatus().isSuccess()) {
				Log.v(LOG_TAG, "DataMap: " + putDataMapReq + " sent successfully to data layer ");
			} else {
				// Log an error
